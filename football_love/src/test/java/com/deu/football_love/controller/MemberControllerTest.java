...
class MemberControllerTest {
  ...
  public void join() throws Exception {
    MemberJoinRequest joinRequest = MemberJoinRequest.memberJoinRequestBuilder().id("dbtlwns").name("유시준").pwd("1234").nickname("개발고수")
        .address(ADDRESS).birth(BIRTH_DAY).email("fblCorp@naver.com").phone("010-1111-2222").type(MemberType.ROLE_NORMAL).build();

    mvc.perform(
        MockMvcRequestBuilders.post(BASE_URL).contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(joinRequest)).with(csrf()))
        .andExpect(status().isOk()).andExpect(jsonPath("$.id", is(joinRequest.getId()))).andExpect(jsonPath("$.name", is(joinRequest.getName())))
  ...
  public void testNotNullJoin() throws Exception {
    MemberJoinRequest joinRequest = MemberJoinRequest.memberJoinRequestBuilder().id("dbtlwns").pwd("1234").nickname("개발고수").address(ADDRESS)
        .birth(BIRTH_DAY).email("fblCorp@naver.com").phone("010-1111-2222").type(MemberType.ROLE_NORMAL).build();
  ...
  public void testPhoneRegexpJoin() throws Exception {
    MemberJoinRequest joinRequest = MemberJoinRequest.memberJoinRequestBuilder().id("dbtlwns").name("유시준").pwd("1234").nickname("개발고수")
        .address(ADDRESS).birth(BIRTH_DAY).email("fblCorp@naver.com").phone("010-1111-22221").type(MemberType.ROLE_NORMAL).build();
  ...
  public void testEmptyStringArgsJoin() throws Exception {
    MemberJoinRequest joinRequest = MemberJoinRequest.memberJoinRequestBuilder().id("dbtlwns").name("").pwd("1234").nickname("개발고수").address(ADDRESS)
        .birth(BIRTH_DAY).email("fblCorp@naver.com").phone("010-1111-2222").type(MemberType.ROLE_NORMAL).build();
  ...
  public void loginTest() throws Exception {
    ...
    MvcResult loginResponse = mvc.perform(MockMvcRequestBuilders.post(BASE_URL + "/login_jwt/" + join.getId()).contentType(MediaType.APPLICATION_JSON)
        .content(mapper.writeValueAsString(loginRequest)).with(csrf())).andExpect(status().isOk()).andDo(print()).andReturn();
    ...
    MvcResult logoutResponse = mvc
        .perform(MockMvcRequestBuilders.post(BASE_URL + "/logout_jwt/").contentType(MediaType.APPLICATION_JSON)
            .content(mapper.writeValueAsString(loginRequest)).cookie(accessTokenCookie).cookie(refreshTokenCookie).with(csrf()))
  ...
  public void chkEmailDuplication() throws Exception {
    ...
    QueryMemberDto join = memberService.join(joinRequest);
    mvc.perform(MockMvcRequestBuilders.get(BASE_URL + "/duplication/email").contentType(MediaType.APPLICATION_JSON).param("email", join.getEmail())
        .with(csrf())).andExpect(status().isConflict()).andDo(print());

    String newEmail = "helloWorld@naver.com";
    mvc.perform(
        MockMvcRequestBuilders.get(BASE_URL + "/duplication/email").contentType(MediaType.APPLICATION_JSON).param("email", newEmail).with(csrf()))
  ...
  public void chkIdDuplication() throws Exception {
    ...
    QueryMemberDto join = memberService.join(joinRequest);
    mvc.perform(
        MockMvcRequestBuilders.get(BASE_URL + "/duplication/id").contentType(MediaType.APPLICATION_JSON).param("id", join.getId()).with(csrf()))
        .andExpect(status().isConflict()).andDo(print());

    String newId = "lilililiil";
    mvc.perform(MockMvcRequestBuilders.get(BASE_URL + "/duplication/id").contentType(MediaType.APPLICATION_JSON).param("id", newId).with(csrf()))
        .andExpect(status().isOk()).andDo(print());
  }
